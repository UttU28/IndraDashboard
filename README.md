...
- **Actions** (no sudo): `Restart`, `Rebuild`, `Stop`, `Start`, `Logs`.
...
Runs on `127.0.0.1:9282`; put nginx in front for a domain (`nginx-dashboard.conf`).
...
covers that combination, so this reads them all from `projects.yaml`
(generated from `~/Desktop/PORTS.md`).

## Setup

```bash
cd ~/Desktop/serverDashboard
python3 -m venv venv
...
The UI can restart/stop services, so protect it. Set HTTP Basic creds via env
(uncomment in `ecosystem.config.cjs`, then `pm2 restart server-dashboard`):
...
compose dir + services, ports, domains, and deploy script. No code changes.

## Port

Uses **9282** (the next free frontend slot in `PORTS.md`). 9283 stays free.
Add a row to `PORTS.md` when you expose it via a domain.
